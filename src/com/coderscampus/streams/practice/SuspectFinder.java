package com.coderscampus.streams.practice;
...
public class SuspectFinder {
	...
	suspects=suspectLineUp.stream().filter(suspect->!suspect.getHairColor().equalsIgnoreCase("RED")).filter(suspect->suspect.getHasGlasses().equals(false)).collect(Collectors.groupingBy(Suspect::getBirthYear,Collectors.toList()));
